...
if st.session_state.script_choice == "tool1":
    ...
    if selected_country != "Custom Weather":
        with col2:
            ...
            if not selected_.empty:
                ...
                # st.write(f"**ASHRAE Climate Zone:** {ashrae_zone}")
            # else:
            #     st.warning("ASHRAE Climate Zone not found")
    else:
        with col2:
    ...
    if st.button("Simulate 🚀"):
        ...
        # LOAD DATA
        # =============================
        exportCSV = resource_path(os.path.join("Exports.csv"))
        ...
        combined_Data["Energy_Saving_%"] = (
            (baseline_energy - combined_Data["Energy_Outcome(KWH)"]) / baseline_energy * 100
        )
        ...
        orient_pct  = calc_energy_reduction(orient_df, baseline_energy)
        ...
        .value-green {
            color: #16a34a;   /* nice modern green */
            font-weight: 600;
        }
        </style>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.markdown(f"""
                ...
                <div class="analysis-grid">
                    <div class="card"><small>Area (ft²)</small><h6>{area_ft2:.1f}</h6></div>
                    <div class="card"><small>Conditioned area</small><h6>{conditioned_pct:.1f}%</h6></div>
                    <div class="card"><small>Window-to-wall ratio</small><h6>{wwr:.2f}</h6></div>
                    <div class="card"><small>Wall / floor area</small><h6>{wall_floor_ratio:.2f}</h6></div>
                    <div class="card"><small>Climate zone</small><h6 style="color:#16a34a;">{ashrae_zone}</h6></div>
                    <div class="card"><small>Above-grade area</small><h6>{above_grade_pct:.1f}%</h6></div>
                    <div class="card"><small>Envelope Area / floor area</small><h6>{roof_floor_ratio:.2f}</h6></div>
                    <div class="card"><small>Estimated hours of use</small><h6>{usage_factor:.1f}</h6></div>
                </div>
            """, 
            unsafe_allow_html=True)
        with col_line:
            st.markdown(
                "<div style='border-left:3px solid red; height:210px'></div>",
                unsafe_allow_html=True
            )
        with col2:
            st.markdown(f"""
                ...
                <div class="analysis-grid">
                    <div class="card"><small>Wall</small><h6>{wall_pct:.1f}%</h6></div>
                    <div class="card"><small>Roof</small><h6>{roof_pct:.1f}%</h6></div>
                    <div class="card"><small>Glazing</small><h6>{glazing_pct:.1f}%</h6></div>
                    <div class="card"><small>WWR</small><h6>{wwr_pct:.1f}%</h6></div>
                    <div class="card"><small>Lighting</small><h6>{light_pct:.1f}%</h6></div>
                    <div class="card"><small>Equipment</small><h6>{equip_pct:.1f}%</h6></div>
                    <div class="card"><small>Energy range (kWh)</small><h6>{range_energy:,.0f}</h6></div>
                    <div class="card"><small>Demand range (kW)</small><h6>{range_demand:.1f}</h6></div>
                </div>
                """, 
            unsafe_allow_html=True)
        # st.write(combined_Data)
        ...
        baseline_energy = combined_Data_expanded.loc[0, "Energy_Outcome(KWH)"]
        combined_Data_expanded["Energy_Saving_%"] = ((baseline_energy - combined_Data_expanded["Energy_Outcome(KWH)"])*100/ baseline_energy)*100
        ...
        def energy_param_plot(
            ...
            fig.add_trace(
                go.Scatter(
                    x=x_param_df[x_col],
                    y=x_param_df["Energy_Outcome(KWH)"],
                    mode="markers",
                    marker=dict(size=12, color="blue", opacity=0.85),
                    customdata=x_param_df["Energy_Saving_%"],
                    hovertemplate=(
                        f"<b>{x_label}</b>: %{{x:.2f}}<br>"
                        "<b>Energy</b>: %{y:,.0f} kWh<br>"
                        "<b>Saving</b>: %{customdata:.1f}%<extra></extra>"
                    ),
                    name="Parametric"
                )
            )
            ...
            fig.update_layout(
                ...
                title=dict(
                    ...
                    font=dict(
                        ...
                        color="black",
                        family="Arial"      # normal (non-bold) font
                    )
                ),
                height=420,
                margin=dict(l=50, r=30, t=70, b=90),  # important for legend space
                font=dict(size=13),
            ...
            fig.update_yaxes(title="Energy Use (kWh)")
            return fig

        fig_wall = energy_param_plot(
            wall_df,
            baseline,
            "R-VAL-W",
            "R-Value (HR·ft²·°F / Btu)",
            "Energy Use vs Wall R-Value",
            show_legend=True
        )
        fig_roof = energy_param_plot(
            roof_df,
            baseline,
            "R-VAL-R",
            "R-Value (HR·ft²·°F / Btu)",
            "Energy Use vs Roof R-Value",
            show_legend=True
        )
        ...
        def make_single_plot(df, x_col, title, x_label):
            ...
            fig.add_trace(go.Scatter(
                ...
                mode="markers",
                name="ECM",
                marker=dict(size=12, color="blue", line=dict(width=1, color="blue")),
        ...
        fig_light = make_single_plot(
            light_df,
            x_col="Light(W/Sqft)",
            title="Energy Use vs Lighting Power",
            x_label="Lighting Power Density (W/ft²)"
        )

        fig_equip = make_single_plot(
            equip_df,
            x_col="Equip(W/Sqft)",
            title="Energy Use vs Equipment Power",
            x_label="Equipment Power Density (W/ft²)"
        )
        ...
        def create_pdf(image_paths, pdf_name="Energy_Parametric_Report.pdf"):
            ...
            elements = []
            elements.append(Paragraph("<b>Energy Parametric Analysis Report</b>", styles["Title"]))
        ...
        if st.button("📄 Generate PDF Report"):
            with st.spinner("Generating PDF..."):
                img_paths = save_figs_as_images(all_figs)
                create_pdf(img_paths)

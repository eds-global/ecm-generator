...
def get_all_calculated_values(user_input, user_nm, typology, inputPath):
    all_data = []
    for file in gb.glob(os.path.join(inputPath, "*.sim")):
        ...
        if len(parts) != 8:
            print(f"[SKIPPED] Folder '{file_name_without_ext}' — expected 11 parts, got {len(parts)}")
            return
    ...
    m = 1
    for file in gb.glob(os.path.join(inputPath, "*.inp")):
        # st.write(file)
        file_name = os.path.basename(file)
        file_name_without_ext = os.path.splitext(file_name)[0]
        path = file_name_without_ext
        zones_files = gb.glob(os.path.join(inputPath, f'{path}_zone_inp.csv'))
        if zones_files:
            zones = pd.read_csv(zones_files[0])
            zones['SPACE'] = zones['SPACE'].astype(str).str.replace('"', '', regex=False)
    
    #     for zone in zip(zones):
    #         # store all csvs info. in dataframe
    #         zoneData = pd.read_csv(zone)
    #         # st.write(zoneData)
    #         zoneData.to_csv("a.csv")
    ...
    for file in gb.glob(os.path.join(inputPath, "*.sim")):
        ...
        for lvbfile, lvdfile, zonefile, summary, lscfile, bepufile, svafile, psefile, shgcfile, lvcfile, lscfile_loss in zip(lvbfiles, lvdfiles, zonefiles, lvd_summary, lscfiles, bepufiles, svafiles, psefiles, shgcfiles, lvcfiles, lscfiles_losess):
            ...
            u_val = summary_data.loc[summary_data['AZIMUTH'] == 'ALL WALLS', 'AVERAGE(U-VALUE/WALLS)(BTU/HR-SQFT-F)'].values[0]
            u_val_roof = summary_data.loc[summary_data['AZIMUTH'] == 'ROOF', 'AVERAGE(U-VALUE/WALLS)(BTU/HR-SQFT-F)'].values[0]
            u_val_wind = summary_data.loc[summary_data['AZIMUTH'] == 'ALL WALLS', 'AVERAGE(U-VALUE/WINDOWS)(BTU/HR-SQFT-F)'].values[0]
            r_val = 1/u_val
            r_val_roof = 1/u_val_roof
            r_value_wind = 1/u_val_wind
            ...
            r_value_roof.append(r_val_roof)
            r_value_window.append(r_value_wind)
            ...
            unconditioned1 = lvb_data.loc[lvb_data['TYPE'] == 'UNCONDITIONED', 'Areas'].sum()
    ...
    result['R-VAL-R'] = r_value_roof
    result['R-VAL-Wind'] = r_value_window

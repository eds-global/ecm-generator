...
def updateCOP(inp_data, cop): 
    ...
    try:
        database = pd.read_excel("database/final_Parameter_DB.xlsx", sheet_name='COP')
        improvement = database['COP'].iloc[cop]
    except FileNotFoundError:
        raise FileNotFoundError("Excel file not found: database/final_Parameter_DB.xlsx")
    except ValueError:
        raise ValueError("Sheet 'COP' not found in the Excel file.")
    except IndexError:
        raise IndexError(f"Invalid 'light' index provided: {cop - 1}. Total rows: {len(database)}")
    ...
    for i in range(start_index, end_index):
        if "LIGHTING-W/AREA" in modified_inp[i]:
            ...
            new_val = improvement
            modified_inp[i] = f"   LIGHTING-W/AREA  = ( {new_val:.1f} )\n"

...
def process_window_insertion_workflow(inp_file_path, row_num): 
    ...
    percentages = [i / 100 for i in range(10, 100, 10)]
    percentages_SH = [i / 100 for i in range(10, 100, 10)]
    ...
    df['X'] = df['D10%']
    df['Y'] = df['SPACE-HEIGHT10%']

    factors = [i / 10 for i in range(1, 9)]  # 0.1 → 0.8 (10% to 80%)
    for i, f in enumerate(factors, start=1):
        factor_sqrt = np.sqrt(f)  # ensures area = factor * wall_area
        df[f'HEIGHT{i}'] = (factor_sqrt * df['SPACE-HEIGHT']).round(2)
        df[f'WIDTH{i}'] = (factor_sqrt * df['D']).round(2)

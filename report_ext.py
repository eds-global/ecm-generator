...
import pandas as pd
import shutil
from src import hvac_system, lv_b, ls_c, lv_c, lv_d, pv_a_loop, sv_a, beps, bepu, lvd_summary, sva_zone, locationInfo, masterFile, sva_sys_type, pv_a_pump, pv_a_heater, pv_a_equip, pv_a_tower, ps_e, inp_shgc
import numpy as np
...
def get_files_for_data_extraction(output_path, log_file_path, new_batch_id, location_id, user_nm, user_input, typology):
    ...
    for file in success_files:
        ...
        try:
            get_report_and_save(hvac_system.get_HVAC_Zone_report, inp_file_path, 'zone_inp', output_path)
            get_report_and_save(inp_shgc.get_SHGC_report, inp_file_path, 'shgc', output_path)
        except Exception as e:
            print(f"Skipping INP!!")
        
    print("extraction complete")
    combined_data = masterFile.get_all_calculated_values(user_input, user_nm, typology, output_path)
    ...
    if combined_data is None:
        ...
        combined_file_path = os.path.join(combined_folder_path, user_nm + '_combined_data.csv')
